...
class SASRec(nn.Module):
    ...
    def forward(self, seq_token, seq_pos):
        ...
        # TrainModel = self.dense(TrainModel)  # 在transformer后面加上一层MLP，因为transformer-encoder最后一层是dropout
        # print('right-transformer-TrainModel-shape:', TrainModel.shape)  # [L,B,H]
...
class GRU4Rec(nn.Module):
    ...
    #     # TrainModel = hidden.view(-1, hidden.shape[2])
    #     TrainModel = self.gather_indexes(gru_output, seq_pos - 1)
    #     # TrainModel = hidden[-1].view(-1, hidden[-1].shape[1])
    #     # TrainModel = self.fc(hidden)
    #     return TrainModel
    def forward(self, seq_token, seq_pos):
        # Supervised Head
        seq_embeddings = self.token_emb(seq_token)
